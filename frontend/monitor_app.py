...
def _gen_alarm_excel(d: dict) -> bytes | None:
    """Alarm sekmesi Excel — Firebase özet verisinden üretilir."""
    try:
        import io
        import openpyxl
        ...
        def _hdr(ws, cols):
            ...
            for cell in ws[1]:
                cell.fill = fill; cell.font = font; cell.alignment = algn
        ...
        # Sheet 1: 7 Günlük Trend
        ws1 = wb.active;  ws1.title = "7 Gunluk Trend"
        ...
        # Sheet 2: Alarm Geçmişi (top alarm kodları)
...
def _gen_alarm_pdf(d: dict, lang: str) -> bytes:
    """Alarm sekmesi PDF — Firebase özet verisinden üretilir. Hata fırlatır."""
    ...
    if lang == "en":
        L = {"title":"Production Report","cycles":"Cycles","anom":"Anomaly Rate",
             ...
             "al":"Alarm","du":"Avg Dur.","co":"Code","na":"Name","ct":"Count","gen":"Generated",
             "daily":"Daily Summary","anom_status":"Anomaly Status","anom_ok":"No Anomaly Detected",
             "anom_warn":"Anomaly Detected","anom_detail":"Detail","total_cycles":"Total Cycles Analyzed"}
    else:
        L = {"title":"Uretim Raporu","cycles":"Cycle Sayisi","anom":"Anomali Orani",
             ...
             "al":"Alarm","du":"Ort. Sure","co":"Kod","na":"Ad","ct":"Sayi","gen":"Olusturulma",
             "daily":"Gunluk Ozet","anom_status":"Anomali Durumu","anom_ok":"Anomali Tespit Edilmedi",
             "anom_warn":"Anomali Tespit Edildi","anom_detail":"Detay","total_cycles":"Analiz Edilen Cycle"}

    if lang == "en":
        L.update({"alarm_hist":"Alarm History","ah_time":"Time","ah_code":"Code",
                  "ah_sev":"Severity","ah_type":"Type","ah_name":"Name",
                  "sev_high":"Critical","sev_med":"Medium","sev_low":"Low"})
    else:
        L.update({"alarm_hist":"Alarm Gecmisi","ah_time":"Zaman","ah_code":"Kod",
                  "ah_sev":"Onem","ah_type":"Tur","ah_name":"Ad",
                  "sev_high":"Kritik","sev_med":"Orta","sev_low":"Dusuk"})

    dd = d.get("daily_stats") or {}; cs = dd.get("cycles",{}); als = dd.get("alarms",{})
    ...
    pdf = FPDF(); pdf.add_page(); pdf.set_auto_page_break(auto=True, margin=12)

    def _sec(title):
        pdf.set_font("Helvetica","B",10); pdf.set_fill_color(26,34,53); pdf.set_text_color(138,184,216)
        pdf.cell(0,7,title,fill=True,**NL); pdf.ln(1)

    # Başlık
    pdf.set_font("Helvetica","B",14); pdf.set_fill_color(26,34,53); pdf.set_text_color(138,184,216)
    pdf.cell(0,11,L["title"],fill=True,align="C",**NL); pdf.ln(3)

    # KPI (4 sütun yan yana)
    _sec(L["daily"])
    kpis = [(L["cycles"],str(cs.get("count",0))),(L["anom"],f'{cs.get("anomaly_rate",0):.1f}%'),
            (L["alarms"],str(als.get("count",0))),(L["dur"],f'{cs.get("avg_duration",0):.0f} s')]
    cw = pdf.epw / 4
    for k,v in kpis:
        x,y = pdf.get_x(), pdf.get_y()
        pdf.set_fill_color(240,244,250); pdf.rect(x,y,cw-2,15,"F")
        pdf.set_xy(x,y+1); pdf.set_font("Helvetica","",7); pdf.set_text_color(100,100,100)
        pdf.cell(cw-2,4,_s(k),align="C")
        pdf.set_xy(x,y+6); pdf.set_font("Helvetica","B",11); pdf.set_text_color(30,80,140)
        pdf.cell(cw-2,7,v,align="C"); pdf.set_xy(x+cw,y)
    pdf.ln(18)
    ...
    if anom_latest:
        ...
        if has_anom:
            pdf.set_fill_color(180,40,40); pdf.set_text_color(255,255,255)
            pdf.cell(0,7,f"! {_s(L['anom_warn'])}",fill=True,border=1,**NL)
            if detail_lines:
                pdf.set_font("Helvetica","",8); pdf.set_fill_color(255,240,240); pdf.set_text_color(40,40,40)
                for line in detail_lines:
                    pdf.cell(0,5,f"  - {_s(str(line))}",fill=True,border=1,**NL)
        else:
            pdf.set_fill_color(20,80,40); pdf.set_text_color(200,255,200)
            pdf.cell(0,7,f"OK  {_s(L['anom_ok'])}",fill=True,border=1,**NL)
        pdf.set_font("Helvetica","I",8); pdf.set_text_color(100,100,100); pdf.set_fill_color(245,245,245)
    ...
    if trend:
        _sec(L["trend"])
        ww=[34,24,24,24,24]; hs=[L["d"],L["cy"],L["an"],L["al"],L["du"]]
        pdf.set_font("Helvetica","B",8); pdf.set_fill_color(220,230,242); pdf.set_text_color(20,20,20)
        for c,w in zip(hs,ww): pdf.cell(w,6,c,border=1,fill=True,align="C")
        pdf.ln(); pdf.set_font("Helvetica","",8)
        for i,r in enumerate(trend):
            pdf.set_fill_color(255,255,255) if i%2==0 else pdf.set_fill_color(242,246,252)
    ...
    if top:
        _sec(L["th"])
        pdf.set_font("Helvetica","B",8); pdf.set_fill_color(220,230,242); pdf.set_text_color(20,20,20)
        for c,w in zip([L["co"],L["na"],L["ct"]],[22,130,18]): pdf.cell(w,6,c,border=1,fill=True,align="C")
        pdf.ln(); pdf.set_font("Helvetica","",8)
        for i,a in enumerate(top):
            pdf.set_fill_color(255,255,255) if i%2==0 else pdf.set_fill_color(242,246,252)
            pdf.set_text_color(40,40,40)
            for v,w in zip([a.get("code",""),a.get("name",""),str(a.get("count",0))],[22,130,18]):
    ...
    if alarm_hist:
        ...
        pdf.set_font("Helvetica","B",8); pdf.set_fill_color(26,34,53); pdf.set_text_color(138,184,216)
        nw = pdf.epw - 34 - 16 - 16 - 20  # ad sütunu kalan genişlik
        for c,w in zip([L["ah_time"],L["ah_code"],L["ah_sev"],L["ah_type"],L["ah_name"]],[34,16,16,20,nw]):
            pdf.cell(w,6,c,border=1,fill=True,align="C")
        pdf.ln(); pdf.set_font("Helvetica","",8)
        for a in alarm_hist:
            sev = a.get("severity") or ""
            pdf.set_fill_color(*SEV_COLORS.get(sev,(248,250,252))); pdf.set_text_color(40,40,40)
    ...
    pdf.set_y(-12); pdf.set_font("Helvetica","I",7); pdf.set_text_color(120,120,120)
...
def _gen_energy_excel(d: dict) -> bytes | None:
    """Enerji sekmesi Excel — Firebase özet verisinden üretilir."""
    try:
        import io
        import openpyxl
        from openpyxl.styles import PatternFill, Font, Alignment
        ...
        wb = openpyxl.Workbook(); tod = d.get("energy_today") or {}

        # Sheet 1: Saatlik Özet + Günlük KPI
        ws1 = wb.active; ws1.title = "Saatlik Ozet"
        ...
        ws1.append([]); ws1.append(["Gunluk Toplam kWh", tod.get("kwh_total",0)])
        ...
        _hdr(ws3, ["Cycle ID", "Baslangic", "kWh/Cycle", "m3/Cycle", "Ort. kW", "Maks. kW", "Ort. m3/h"])
        for r in _fb_list(d.get("cycle_energy")):
            ws3.append([r.get("cycle_id",""), r.get("start_time",""), r.get("kwh_cycle",0),
                        r.get("nm3_cycle",0), r.get("kw_mean",0), r.get("kw_max",0),
...
def _gen_energy_pdf(d: dict, lang: str) -> bytes:
    """Enerji sekmesi PDF — Firebase özet verisinden üretilir. Hata fırlatır."""
    from fpdf import FPDF
    from fpdf.enums import XPos, YPos
    from datetime import date as _dt
    ...
    if lang == "en":
        L = {"title":"Energy Consumption Report","kpi":"Daily Summary","kwh":"Total kWh",
             ...
             "hourly":"Hourly Trend (Last 24h)","hhr":"Hour","hkwh":"Avg kW","ham2":"Avg m3/h",
             "gen":"Generated"}
    else:
        L = {"title":"Enerji Tuketim Raporu","kpi":"Gunluk Ozet","kwh":"Toplam kWh",
             ...
             "hck":"kWh/Cycle","hcn":"m3/Cycle","hmk":"Maks. kW","ham":"Ort. m3/h",
             "hourly":"Saatlik Trend (Son 24 Saat)","hhr":"Saat","hkwh":"Ort. kW","ham2":"Ort. m3/h",
             "gen":"Olusturulma"}
    ...
    for k,v in [(L["kwh"],f'{tod.get("kwh_total",0):.2f} kWh'),(L["nm3"],f'{tod.get("nm3_total",0):.3f} m3'),
                (L["kwht"],f'{tod.get("kwh_per_tire",0):.3f} kWh/tire'),(L["nm3t"],f'{tod.get("nm3_per_tire",0):.4f} m3/tire')]:
    ...
    if et:
        ...
        for c,w in zip([L["hhr"],L["hkwh"],L["ham2"]],[50,65,65]): pdf.cell(w,7,c,border=1,fill=True,align="C")
        pdf.ln(); pdf.set_font("Helvetica","",9)
        for i,r in enumerate(et):
            pdf.set_fill_color(255,255,255) if i%2==0 else pdf.set_fill_color(240,248,255)
            pdf.set_text_color(40,40,40)
            for v,w in zip([str(r.get("hour","")),f'{r.get("kw_mean",0):.2f}',f'{r.get("air_flow_mean",0):.2f}'],[50,65,65]):
    ...
    if sv:
        ...
        pdf.set_font("Helvetica","B",9); pdf.set_fill_color(220,230,242); pdf.set_text_color(20,20,20)
        for c,w in zip([L["hs"],L["hk"],L["hn"],L["hkw"],L["hr"]],[30,32,32,32,32]): pdf.cell(w,7,c,border=1,fill=True,align="C")
        pdf.ln(); pdf.set_font("Helvetica","",9)
        for i,r in enumerate(sv):
            pdf.set_fill_color(255,255,255) if i%2==0 else pdf.set_fill_color(240,248,255)
            pdf.set_text_color(40,40,40)
            for v,w in zip([r.get("shift",""),f'{r.get("kwh_total",0):.3f}',f'{r.get("nm3_total",0):.3f}',
                            f'{r.get("kw_mean",0):.1f}',f'{r.get("running_pct",0):.1f}%'],[30,32,32,32,32]):
                pdf.cell(w,6,_s(str(v)),border=1,fill=True,align="C")
            pdf.ln()
        pdf.ln(4)
    ...
    if ec:
        ...
        pdf.set_font("Helvetica","B",8); pdf.set_fill_color(220,230,242); pdf.set_text_color(20,20,20)
        for c,w in zip([L["hci"],L["hst"],L["hck"],L["hcn"],L["hkw"],L["hmk"],L["ham"]],[18,42,28,28,28,28,18]):
        ...
        for i,r in enumerate(ec[-20:]):
            ...
            for v,w in zip([str(r.get("cycle_id","")),str(r.get("start_time",""))[:16],
                            f'{r.get("kwh_cycle",0):.3f}',f'{r.get("nm3_cycle",0):.4f}',
                            f'{r.get("kw_mean",0):.1f}',f'{r.get("kw_max",0):.1f}',
                            f'{r.get("air_flow_mean",0):.1f}'],[18,42,28,28,28,28,18]):
                pdf.cell(w,6,_s(str(v)),border=1,fill=True,align="C")
            pdf.ln()
    ...
    with _c2:
        with st.container(border=True):
            ...
            with st.form("login_form"):
                st.html(f'<p style="color:#1a2540;font-size:0.85rem;font-weight:600;margin:0 0 4px 0;">{_u["login_user"]}</p>')
                _user = st.text_input("u", placeholder=_u["login_user"].lower(), label_visibility="collapsed")
                st.html(f'<p style="color:#1a2540;font-size:0.85rem;font-weight:600;margin:8px 0 4px 0;">{_u["login_pass"]}</p>')
                _pass = st.text_input("p", type="password", placeholder="••••••••", label_visibility="collapsed")
